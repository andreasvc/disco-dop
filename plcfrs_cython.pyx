...
#except: pass
myintern = {}
def parse(sent, grammar, str start="S", viterbi=False, n=1):
	...
	goal = interntuple(start, (2**len(sent) - 1,))
	m = maxA = 0
...
cdef inline deduced_from(Ih, double x, C, unary, binary):
	I, Ir = Ih
	result = []
...
cdef inline concat(node):
	...
	#result = tuple(reduce(lambda a,b: a|b.pop(), s, 0) for s in node)
	r = [[a.pop() for a in b] for b in node]
	result = tuple([foldor(a) for a in r])
	return result if contiguous else None

cdef inline foldor(s):
